...
// `sustainMs`, when present, overrides SETTINGS.sustainMs.value for this
// song. Needed because a raw oscillator (synth) holds at FULL gain for the
// whole programmed duration, unlike a piano sample, which decays naturally
// on its own regardless of what durationMs says. Sharing the piano's
// ~1200ms default with the synth route means fast passages overlap at full
// volume - audibly a "flanging/echo" clash, not a bug in the scheduler.
// Keep this shorter than the typical gap between notes for the piece.
...
export function resolveEngineRoute(
  seq,
  { songId = null, routingEnabled = true } = {},
) {
  if (!routingEnabled) {
    return {
      engine: "piano",
  ...
  if (override) {
    return {
      ...
      // null = "no override, use the caller's own sustainMs" - only synth
      // overrides typically set this; piano overrides (if any) usually
      // don't need to.
      sustainMs: Number.isFinite(override.sustainMs)
        ? override.sustainMs
        : null,
  ...
  if (hintMatchesSynth(seq)) {
    return {
      engine: "synth",
      waveform: "sawtooth",
      // Same overlap issue applies to auto-detected synth routes, not just
      // the explicit overrides above - a raw oscillator held at the
      // piano-tuned default would clash the same way.
      sustainMs: 300,
